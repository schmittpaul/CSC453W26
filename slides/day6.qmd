...
# Midterm boundary {background-color="#907845"}
...
- [Hardware support]{.alert}: modern CPUs support multithreading (hyperthreading), where threads can run in parallel
...
- What happens if a thread closes a file another expects to be open?
...
- Example: $S = 50%$. You are capped at $2 \times$ speedup
...
- Pthreads can be used as a [user space]{.alert} library: all functionality happens in user space, and the kernel knows nothing about them
...
- Where we typically want threads is where there is a lot of I/O blocking or system calls: the web server example
  - if we're trapping to the kernel, are we really gaining anything?

## Kernel threads
- Thread libraries can also have kernel support (most modern OSes support kernel threads)
...
- Number of kernel-level threads is equal to the number of user-level threads
...
- Embedded system
  - N:1 - In embedded systems, simplicity and determinism matter more than scalability. You may only have a single core
- NGINX web servers
