...
- cgroups and namespaces are [mechanisms]{.alert}, containers allow us to apply [policy]{.alert} by defining how we'll use them
- Containers combine namespaces + cgroups + layered filesystems
...
- [Synchronization]{.alert}: should `send()` and `recv()` block or not?
- [Buffering]{.alert}: how much and how long do we store messages?
- [Overhead]{.alert}: every message requires two system calls (`send()` & `recv()`); passing a message is slower than controlling access to shared memory (messages are copied)
- [Reliability]{.alert}: e.g., acknowledgement in the face of lost messages; out-of-order of delivery
- [Authentication]{.alert} of messages
